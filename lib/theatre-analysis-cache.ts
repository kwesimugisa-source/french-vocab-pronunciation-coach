...
export class TheatreAnalysisCache {
  ...
  get(reference: unknown, source: string): DramaticAnalysis | undefined {
    this.prune();
    if (typeof reference !== "string") return undefined;
    const entry = this.entries.get(reference);
    return entry?.digest === createHash("sha256").update(source).digest("hex") ? structuredClone(entry.analysis) : undefined;
  }
  put(source: string, analysis: DramaticAnalysis): string {
    this.prune();
    while (this.entries.size >= this.capacity) this.entries.delete(this.entries.keys().next().value!);
    const reference = randomUUID();
    this.entries.set(reference, { digest: createHash("sha256").update(source).digest("hex"), analysis: structuredClone(analysis), expires: this.now() + 30 * 60_000 });
    return reference;
  }
